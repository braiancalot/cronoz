...
The hook's `onClick` also `preventDefault`s and `stopPropagation`s — the `ProjectCard` trigger sits inside a `<Link>` and would otherwise navigate. Don't add a second `onClick` to the trigger button; it would override the hook's.
